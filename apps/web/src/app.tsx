...
function TodaySetup({
  fetcher,
  getAccessToken,
}: Required<Pick<BootstrapOptions, "fetcher" | "getAccessToken">>) {
  ...
  useEffect(() => {
    fetcher("/v1/sessions", {
      ...
      .catch(async () => {
        setCached(await loadLatestIssuedGame().catch(() => undefined));
        setError(true);
      });
  ...
  async function begin() {
    ...
    try {
      ...
      const startResponse = await fetcher("/v1/plays/start", {
        ...common,
        method: "POST",
        body: JSON.stringify({ mode: "daily", hintMode }),
      });
      if (!startResponse.ok) {
        const problem = await startResponse.json().catch(() => ({}));
        if (problem?.code === "CLOCK_ROLLBACK") setClockError(true);
...
function ArchivePage({ fetcher }: { fetcher: typeof fetch }) {
  ...
  async function play(day: string) {
    const entry = entries.find((e) => e.gameDay === day)!;
    try {
      const response = await fetcher("/v1/plays/start", {
        method: "POST",
        credentials: "include",
        ...
        body: JSON.stringify({
          ...
          boardVersion: entry.version,
        }),
      });
      if (!response.ok) throw Error();
      const started = StartedGameSchema.parse(await response.json());
      setGame({ board: started.board, start: started.play });
    } catch {
      setError(
        "We couldn’t start that Archive game. Check your device date and try again.",
      );
    }
  }
