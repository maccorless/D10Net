...
test("plays an 11-point Hints Off Daily in one viewport", async ({ page }) => {
  ...
  await expect
    .poll(async () =>
      Number(
        (
          await sql`select score from plays where mode='daily' and finished_at is not null order by finished_at desc limit 1`
        )[0]?.score,
      ),
    )
    .toBe(11);
...
test("continues offline after start and retries the queued finish on reconnect", async ({
  page,
}) => {
  const sql = postgres(process.env.TEST_DATABASE_URL!);
  ...
  const before = Number(
    (
      await sql`select count(*) n from plays where mode='daily' and finished_at is not null`
    )[0].n,
  );
