...
export async function resetTestDatabase() {
  if (process.env.NODE_ENV !== "test")
    throw new Error("The deterministic seed is test-only");
  ...
  for (const name of migrations) {
    const source = await readFile(
      resolve(process.cwd(), `apps/api/src/db/migrations/${name}`),
      "utf8",
    );
    for (const statement of source
      .split(name === "0001_initial.sql" ? "--> statement-breakpoint" : ";")
      .map((value) => value.trim())
      .filter(Boolean))
      await sql.unsafe(statement);
  }
  await sql.unsafe(
    "create table migrations (id serial primary key, name text unique not null, executed_at timestamp default current_timestamp)",
  );
  for (const name of migrations)
  ...
  for (const [id, day] of [
    ["largest-cities", "2026-07-11"],
    ["past-cities", "2026-07-10"],
    ["missed-cities", "2026-07-09"],
    ["emergency-cities", "2026-07-12"],
  ] as const) {
  ...
  await sql.end();
  console.log(
    "Seeded canonical day 2026-07-11, publisher, daily, archive, and emergency boards",
  );
}
